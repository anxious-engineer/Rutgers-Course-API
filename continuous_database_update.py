...
# Runs Updates indefinetly - SOON TO BE UPDATED
while True :

	# Determines if data has been changed requiring a commit to the DB
	needsCommit = False

	# Stores the number of courses whose status' are checked.
	totalCourses = 0
	# Stores the number of courses that are updated.
	...
	for sub in subjects:
		...
		for course in allCourses:
			...
			# Updates the total courses count.
			totalCourses += 1
			...
			# Begins iteration over every section in the  section json Array.
			for section in sections:
				...
				if needsUpdate:
					...
					# New data needs to be commited, updates needsCommit accordingly
					needsCommit = True

					# Updates the new course updates count.
					totalNewCourseUpdates += 1
